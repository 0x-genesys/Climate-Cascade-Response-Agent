...
| Baseline, implemented and evaluated | One OpenAI-compatible structured completion receives the frozen dossier and scenario. It writes draft actions only, has no tools, retries, memory, verifier, human-feedback loop, or life-safety estimator. A deterministic evaluator checks response structure, evidence IDs, policy patterns, and LSAC@5 from explicit human adjudication. | One credentialed `gpt-5-mini-2025-08-07` run completed with five actions. Human adjudication measured LSAC@5 `3/17` (`17.65%`): only Bharatpur's pending-data-gap requirement was covered. Unsafe autonomous actions `0`; missing evidence references `0`; valid evidence references `9`; runtime `40.92s`; tokens `1,702` prompt and `1,950` completion; cost not captured. Evidence: `runs/baseline/nepal-emsr927-v1.{run,adjudication,evaluation}.json`; `docs/execution/2026-08-30-nepal-baseline-evaluation.md`. | Keep as the fair direct-prompt baseline. It is safe by the implemented policy checks but weak at location-specific operational coverage: generic Rasuwa and Trishuli actions missed Timure access, Bidur residential triage, and Syapru Besi critical-services continuity. Iteration 1 must retrieve and verify AOI-specific evidence before action drafting. |
