...
| Baseline compatibility correction | A credentialed `gpt-5-mini` call failed because the gateway sent unsupported `temperature: 0`. Removed that parameter, added a payload regression test, and split saved-run scoring into `climate-cascade-evaluate-baseline` so manual adjudication never triggers another model call. | Initial model call: HTTP `400`, no actions or LSAC@5. Focused contract and evaluator tests pass. The isolated worktree lacks the user credential, so no post-fix model call was made here. | Keep. The API compatibility defect is fixed, but model quality remains unmeasured until the credentialed shell reruns the baseline and a human adjudicates the saved output. |
| Baseline, measured on Nepal challenging case | Ran one post-fix credentialed `gpt-5-mini-2025-08-07` baseline call and human-adjudicated the five draft actions against the frozen four-action gold set. | LSAC@5 `3/17` (`17.65%`): only Bharatpur's pending-data-gap requirement was covered. Unsafe autonomous actions `0`; missing evidence references `0`; valid references `9`; runtime `40.92s`; tokens `1,702` prompt and `1,950` completion; cost not captured. | Keep as the fair direct-prompt baseline. The key limitation is location-specific operational coverage: generic Rasuwa and Trishuli actions did not satisfy Timure access, Bidur residential triage, or Syapru Besi critical-services requirements. Build the next workflow to retrieve and verify AOI-specific evidence before action drafting. |
