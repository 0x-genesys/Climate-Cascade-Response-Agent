...
The dashboard's primary demo path selects real CEMS examples for Nepal, Poland, and Sri Lanka. After a live response run pins evidence, a paired direct-prompt baseline can reuse that exact immutable source artifact without receiving the deterministic impact package. Frozen fixtures remain the reproducible regression and formal-scoring path.
...
- static HTML, CSS, and JavaScript served by FastAPI for a portable local demo
...
Implementation status: the single-call structured baseline, deterministic evaluator, SQLite persistence, content-addressed artifact store, FastAPI run control plane, ordered SSE progress stream, leased worker workflow, local `uv run climate-cascade-local` setup/startup path, CEMS source adapter, versioned source-evidence package, configured response supervisor, deterministic draft checks, response-review dashboard, and captured-live baseline comparison path are implemented. Iteration 2 adds a deterministic CEMS product-statistics impact package: it chooses one newest finished product per AOI, extracts source-reported population, residential buildings, facilities, roads, bridges, and gaps, and prevents duplicate product-version sums. The captured-live baseline reuses the agent's exact source artifact but does not receive those impact facts. This controls source freshness, but it needs a pre-frozen rubric and human adjudication before it can establish an LSAC delta. It does not run local raster/vector overlays, numeric life-safety estimates, or human approval decisions. See [the paired comparison bundle](../runs/live_comparison/README.md) and [the execution record](execution/2026-08-31-17-live-baseline-comparison.md).

### Iteration 1: verified event intake

Implemented for CEMS Rapid Mapping activations: a typed adapter retrieves and canonicalizes the public activation response, records a SHA-256 snapshot, publisher, URL, retrieval time, and license note, then produces typed claims, findings, AOI product status, and data gaps. When source verification succeeds, the worker calls the configured response supervisor once with compact evidence and scenario constraints, stores its exact response, normalizes a known source-ID citation alias to the immutable snapshot ID, validates cited evidence IDs and action limits, performs deterministic action checks, and pauses at human review. The dashboard exposes every stage through persisted SSE, including real source-intake, draft-generation, structured-response, and deterministic-check milestones. It deliberately does not present fabricated token streaming or private model reasoning. It describes the selected disaster in plain language and renders stored evidence, official product links, accepted draft actions, evaluation state, and a flow-linked glossary for non-technical reviewers.

Observed source-intake result: live `EMSR927` was correctly marked `preliminary` because the activation is open and two products were waiting for data. Historical flood activations `EMSR756` and `EMSR851` were marked `supported` with no source-level data gaps. Those runs were captured before the response supervisor existed. The new supervisor and deterministic evaluation contracts pass with a static test gateway, but no credentialed agent output has been adjudicated, so evidence precision and action coverage remain unmeasured. See [Iteration 1 evidence](../runs/iteration_1/) and [the continuation record](execution/2026-08-31-07-iteration-1-response-supervisor-dashboard.md).
