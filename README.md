...
An emergency operations analyst or incident commander working after a flood or debris-flow event has been verified. They must turn evolving mapping, hazard, population, infrastructure, and access information into a short set of defensible actions for human-led follow-up. Mapping teams, humanitarian logistics coordinators, public-works officials, and health-service coordinators support that same decision.

### 02. What Bottleneck Makes It Worth Solving?

The evidence arrives in fragments and changes over time. The analyst has to reconcile verified facts with preliminary claims, identify where map products are still missing, avoid treating exposed population as lives saved, and assign the next reviewable action an owner, location, evidence, and time window. A damage map alone does not produce that decision trail, and a fluent summary can hide uncertainty or propose unsupported action.

### 03. Does The Agent Solve It Well?

Partially, with evidence and clear limits. The prompt-only baseline scored LSAC@5 `0/17` on the frozen Nepal challenge. On retained live CEMS diagnostic runs, deterministic product facts plus constrained planning reached `13/17` in Iteration 2 and `14/17` in Iteration 3, while automatic checks found `0` unsafe autonomous actions and `0` missing evidence references. Iteration 4 retained `13/17` because it improves human-control records after planning, not draft selection. These live results are not a fair baseline uplift claim: their CEMS data is mutable and the rubric transfer was project-owner, AI-assisted. The agent remains decision support only. It never dispatches, warns, evacuates, or executes an action.
